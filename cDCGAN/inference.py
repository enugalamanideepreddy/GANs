...
if __name__ == "__main__":
    ...
    parser.add_argument('--class_number', type=str, required=True, help='Which class to generate image.(from airplane, automobile, horse, ship) ex : 0,1,2,3')
    ...
    latent = torch.randn((args.num_images,config.latent_vector_size),device = args.device)
    labels = torch.full(size = (args.num_images,),fill_value=int(args.class_number),device = args.device)
    
    out = gen_model(latent,labels)
